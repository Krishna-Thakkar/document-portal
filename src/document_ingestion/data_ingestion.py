...
class FaissManager:
    ...
    @staticmethod
    def _fingerprint(text: str, md: Dict[str, Any]) -> str:    # to avoid de-duplication in vectorstore
...
class ChatIngestor:
    def __init__(self,
        ...
        try:
            ...
            raise DocumentPortalException(f"Initialization error in ChatIngestor", e) from e
    ...
    def build_retriever(self,
        ...
        *,                           # * indicates that all arguments after it must be passed as keyword arguments     
        ...
        try:
            ...
            raise DocumentPortalException(f"Failed to build retriever", e) from e
...
class DocHandler:
    ...
    def read_pdf(self, pdf_path: str) -> str:
        try:
            ...
            raise DocumentPortalException(f"Could not process PDF: {str(e)}", e) from e
...
class DocumentComparer:
    ...
    def __init__(self, base_dir: str = "data/document_compare", session_id: Optional[str] = None):
        ...
        log.info("DocumentComparer initialized", session_path=str(self.session_path))
    ...
    def save_uploaded_files(self, reference_file, actual_file):
        try:
            ...
            log.info("Files saved", reference=str(ref_path), actual=str(act_path), session_id=self.session_id)
            return ref_path, act_path
        
        except Exception as e:
            log.error("Error saving PDF files", errors=str(e), session_id=self.session_id)
            raise DocumentPortalException(f"Error saving files: {str(e)}", e) from e
    ...
    def combine_documents(self):
        try:
            ...
            combined_text = "\n\n".join(doc_parts)
            log.info("Document combined", count=len(doc_parts), session=self.session_id)
